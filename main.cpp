...
void main()
{
	...
	cin.getline(filename_after, n);
	add_file_dhcpd(filename_after);
	cout << "Введите номер аудитории: ";cin>> number_of_room;
	ofstream fout(filename_after);
	ifstream fin("201 Raw.txt");
	int computer_num;
	std::cout << "Введите количество компьютеров в аудитории от 0 до 12: "; 
	do
	{
		std::cin >> computer_num;
	} while (!(computer_num >= 0 && computer_num <= 12));
	...
	if (fin.is_open())
	{
		...
		{*/
			...
			for (int i = 0; i < computer_num; i++)
			{
				fin >> sz_ip;
				fin >> sz_mac;
				fout << "host "<<number_of_room<<"-" << (i+1)  << endl;
				fout << "{" << endl;
				fout << "\t\t\t hardware ethernet: " << sz_mac << endl;
				fout << "\t\t\t fixed-address:     " << sz_ip << endl;
				fout << "}" << endl;
			}
